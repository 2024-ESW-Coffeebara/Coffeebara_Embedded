...
def results_processing(largest_cup, largest_entrance, largest_holder):
    ...
    if entrance_2_cup >= 400:
        print("Cup size : Large")
        commend = commend | 0x03
    elif entrance_2_cup < 400 and entrance_2_cup > 350:
    ...
    if entrance_x_diff >= 280:
        ...
        commend = commend | 0x03
    elif entrance_x_diff < 280 and entrance_x_diff >= 258:
...
def state_update():
    global RESET, READY, START, DETECTING, DETECTED, model_path, webcam, current_state, model, image_index
    global tx_rx_data
    tx_rx_data = 0x00
    ...
    if py_serial.readable():
        ...
        if current_state == PY_RESET:
            ...
            frame = frame[:, : 400]
            frame_name = f"image_{image_index}.png"
            frame_path = os.path.join(output_folder, frame_name)

            cv2.imwrite(frame_path, frame)

            print(f"{frame_path}에 이미지가 저장되었습니다.")
            image_index += 1

            results = model(frame,imgsz = (640,640))
            ...
            while True:
                ...
                print(response)
                if response == HORIZONTAL_WASTE_CUP:
...
if __name__ == "__main__":
    ...
    socketio.run(app, host = 'localhost', port = 5113, debug = True)